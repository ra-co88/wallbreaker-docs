...
<Callout type="info">104 tools registered across 72 modules in `wallbreaker/tools/`. Use `/tools` in the TUI to see them live during a session.</Callout>
...
| Tool | Description |
|------|-------------|
| `_get` | Fetch a jailbreak seed from  by file name, or name='all' for every file. |
| `_list` | List the jailbreak seed files in the  library. |
| `_search` | Keyword-search the  jailbreak library across all seed files. |
